...
export function createScrollAnimation(
    ...
    return gsap.fromTo(element, config.from, {
        ...
        ease,
        stagger:
            (Array.isArray(element) || element instanceof NodeList) && (element as ArrayLike<Element>).length > 1
                ? stagger
                : undefined,
        scrollTrigger: {
...
export function gsapScrollTrigger(
    ...
    const opts: ScrollTriggerOptions =
...
export function textReveal(
    ...
    const { splitBy = 'words', stagger = 0.05, duration = 0.6, delay = 0, ease = easings.smooth } = options;
    ...
    element.innerHTML = parts
        .map((part, i) => `<span class="text-reveal-part" style="display: inline-block; overflow: hidden;"><span class="text-reveal-inner" style="display: inline-block;">${part}${splitBy === 'words' && i < parts.length - 1 ? '&nbsp;' : ''}</span></span>`)
        .join('');
